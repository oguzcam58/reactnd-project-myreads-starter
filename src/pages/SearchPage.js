    ...
    if (!query || query.length < 1) {
      ...
      BooksAPI.search(query).then(res => {
        if (query === this.state.query) {
          this.setState({ apiCallInProgress: false, errorOccurred: false, results: res });
        }
      }).catch(() => {
        if (query === this.state.query) {
          this.setState( { apiCallInProgress: false, errorOccurred: true, results: []} );
        }
      });
    }
  }
    ...
    return (
      <div className="search-books">
        ...
        <div className="search-books-results">
          { !apiCallInProgress && errorOccurred && (
